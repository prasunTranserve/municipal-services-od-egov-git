...
@RestController
@Slf4j
public class WSSchedulerTriggerController {
	...
	@Autowired
	private MigrationUtility utility;
	
	@Autowired
	@Qualifier("stepWsGenerateBill")
	Step stepWsGenerateBill;
	
	@PostMapping("/ws/generatebill")
	public void wsFetchbill(@RequestBody @Valid MigrationRequest request) throws InvalidFormatException, IOException {
		...
		properties.setBulkBillCriteria(request.getBulkBillCriteria());
			...
			if(fileToProceed.isFile() && fileToProceed.getName().endsWith(".xlsx")) {
		        ...
		        try {
		        	...
		        	Job job = jobBuilderFactory.get("firstBatchJob")
		        			...
		        			.flow(stepWsGenerateBill).end().build();
