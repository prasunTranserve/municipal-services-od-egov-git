...
public class WSCalculationValidator {
	...
	public void validateMeterReading(MeterConnectionRequest meterConnectionRequest, boolean isUpdate) {
		...
		if (!CollectionUtils.isEmpty(previousMeterReading)) {
			...
			if (meterReading.getCurrentReading() < currentMeterReading && !(meterReading.getMeterStatus().equals(MeterStatusEnum.RESET))) {
		...
		if (meterReading.getCurrentReading() < meterReading.getLastReading() && !(meterReading.getMeterStatus().equals(MeterStatusEnum.RESET))) {
